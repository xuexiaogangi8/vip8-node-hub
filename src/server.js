...
const PUBLIC_BASE_URL = process.env.PUBLIC_BASE_URL || 'https://sub.vip8.tech';
...
app.get('/', requireMember, (req, res) => {
  ...
  const url = sub ? `https://sub.vip8.tech/sub/${sub.token}` : '-';
...
app.get('/admin', requireAdmin, (req, res) => {
  ...
  const subCards = subscriptions.map((s) => {
    const url = `https://sub.vip8.tech/sub/${s.token}`;
...
function adminScripts() { return `<script>
  ...
  async function forceResetUserSub(id,username){if(!confirm('确认强制更新 '+username+' 的订阅地址吗？旧地址会立刻失效。')) return;const r=await fetch('/api/users/'+id+'/reset-subscription',{method:'POST',headers:{'Content-Type':'application/json'},credentials:'same-origin'});const j=await readJsonSafe(r);if(!j.ok){alert('失败: '+j.error);return;}const url=j.url||('https://sub.vip8.tech/sub/'+j.item.token);prompt('新的完整订阅地址（已可复制）', url);location.reload();}
  async function copyUserSub(token,username){if(!token){alert('这个用户还没有订阅地址');return;}const url='https://sub.vip8.tech/sub/'+token;try{if(navigator.clipboard&&window.isSecureContext){await navigator.clipboard.writeText(url);alert(username+' 的订阅地址已复制');return;}}catch(_){ }prompt(username+' 的当前订阅地址（可复制）', url);}
